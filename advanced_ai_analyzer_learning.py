import os
# Set tokenizers parallelism before importing transformer libraries
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        context = context.transpose(1, 2).reshape(batch_size, 1, -1).squeeze(1)
        return self.out_proj(context)

class CrossAttention(nn.Module):
    """Cross-attention mechanism for connecting different representations"""
    ...
    def forward(self, query_input, key_value_input):
        # query_input shape: (batch_size, hidden_size)
        # key_value_input shape: (batch_size, hidden_size)
...
class DomainAdapter(nn.Module):
    """Domain-specific adapter for specialized research areas"""
...
class EnhancedClassifier(nn.Module):
    """Enhanced neural network classifier with improved attention, residual connections and domain adapters"""
    
    def __init__(self, input_size, hidden_size, num_classes, num_residual_blocks=6, num_heads=8):
        super(EnhancedClassifier, self).__init__()
        
        self.input_projection = nn.Linear(input_size, hidden_size)
        self.self_attention = SelfAttention(hidden_size, num_heads=num_heads)
        ...
        # Domain adapters for specialized research areas
        self.domain_adapters = nn.ModuleList([
            DomainAdapter(hidden_size) for _ in range(3)  # For ML, NLP, and CV domains
        ])
        
        # Add hierarchical attention
        self.hierarchical_attention = nn.ModuleList([
            SelfAttention(hidden_size, num_heads=num_heads),
            SelfAttention(hidden_size, num_heads=num_heads)
        ])
    ...
    def forward(self, x, domain_id=None):
        ...
        # Apply first half of residual blocks
        for i in range(len(self.residual_blocks) // 2):
            x = self.residual_blocks[i](x)
        
        # Apply hierarchical attention in the middle
        h_attn1 = self.hierarchical_attention[0](x)
        x = x + h_attn1  # Skip connection
        
        # Apply domain adapter if specified
        if domain_id is not None and 0 <= domain_id < len(self.domain_adapters):
            x = self.domain_adapters[domain_id](x)
        
        # Apply second half of residual blocks
        for i in range(len(self.residual_blocks) // 2, len(self.residual_blocks)):
            x = self.residual_blocks[i](x)
        
        # Apply final hierarchical attention
        h_attn2 = self.hierarchical_attention[1](x)
        x = x + h_attn2  # Skip connection
...
class LearningSystem:
    ...
    def evaluate(self, model_type='enhanced'):
        ...
        try:
            ...
            if not self._prepare_training_data():
                logger.error("No valid evaluation data available")
                return {
                    'overall_f1': 0.0,
                    'accuracy': 0.0,
            ...
            # Get the model from models dictionary
            ...
            # If we have too few samples, just return accuracy based on training data
            if len(X_eval) < 5:
                logger.warning("Too few samples for proper evaluation, reporting training accuracy")
                return {
                    'overall_f1': 0.5,  # Placeholder values
                    'accuracy': 0.5,
                    'status': 'insufficient_data'
                }
            
            # Create dataset and dataloader
            eval_dataset = PaperDataset(X_eval, y_eval)
            ...
            if len(all_labels) > 0 and len(all_preds) > 0:
                ...
                try:
                    ...
                    # Fallback if precision_recall_fscore_support fails
                    precision, recall, f1 = 0, 0, 0
                
                # Get per-category metrics if possible
                category_metrics = {}
                if len(np.unique(all_labels)) > 1 and len(np.unique(all_preds)) > 1:
                    try:
                        ...
                        for idx, cls_f1 in enumerate(class_f1):
                            if idx < len(self.categories):
                                cat_name = self.categories[idx]
                                category_metrics[cat_name] = float(cls_f1)
                    except:
                        pass
                ...
                logger.warning("No predictions made during evaluation")
                return {
                    'overall_f1': 0.0,
                    'accuracy': 0.0,
            ...
            return {
                'overall_f1': 0.0,
                'accuracy': 0.0,
                'status': f'evaluation_error: {str(e)}'
            }
