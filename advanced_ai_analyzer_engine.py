...
class RecursiveResearchEngine:
    ...
    def run(self, initial_queries, iterations=5, max_results=1000):
        ...
        # Phase 3: Self-improvement and knowledge consolidation
        logger.info("Phase 3: Self-improvement and knowledge consolidation")
        
        if self.research_frontiers:
            logger.info(f"Consolidating knowledge around {len(self.research_frontiers)} identified research frontiers")
            self._consolidate_knowledge()
        else:
            # Even if no specific frontiers were identified, consolidate around top concepts
            logger.info("No specific research frontiers identified, consolidating around top concepts")
            top_concepts = self.kb.get_top_concepts(n=5)
            if top_concepts:
                self.research_frontiers = top_concepts
                logger.info(f"Using top {len(top_concepts)} concepts as research frontiers for consolidation")
                self._consolidate_knowledge()
            else:
                logger.info("No concepts available for consolidation, skipping consolidation phase")
    ...
    def _consolidate_knowledge(self):
        logger.info("Consolidating knowledge around identified research frontiers")
        # Implementation of _consolidate_knowledge method
        pass
