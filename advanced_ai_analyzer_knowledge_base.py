    ...
    def add_papers(self, processed_papers):
        ...
        if embeddings_to_add:
            ...
            if isinstance(self.faiss_index, faiss.IndexIVFFlat) and not self.faiss_index.is_trained:
                try:
                    # For small datasets, we might not have enough vectors initially
                    # Create a larger training set by repeating and adding noise
                    min_training_vectors = 100
                    if len(embeddings_np) < min_training_vectors:
                        logger.info(f"Only {len(embeddings_np)} vectors available, augmenting for training")
                        # Replicate vectors with small noise to meet minimum size
                        repeats_needed = int(np.ceil(min_training_vectors / len(embeddings_np)))
                        training_vectors = []
                        for _ in range(repeats_needed):
                            # Add small random noise to avoid identical vectors
                            noisy_vectors = embeddings_np + np.random.normal(0, 0.01, embeddings_np.shape).astype(np.float32)
                            training_vectors.append(noisy_vectors)
                        # Combine and use for training
                        training_np = np.vstack(training_vectors)
                        # Trim to exactly min_training_vectors
                        training_np = training_np[:min_training_vectors]
                        logger.info(f"Training FAISS IVF index with {len(training_np)} augmented vectors")
                        self.faiss_index.train(training_np)
                    else:
                        logger.info(f"Training FAISS IVF index with {len(embeddings_np)} vectors")
                        self.faiss_index.train(embeddings_np)
                    logger.info("FAISS index successfully trained")
                except Exception as e:
                    logger.error(f"Failed to train FAISS index: {e}")
                    # Fall back to flat index if training fails
                    try:
                        logger.info("Falling back to flat index due to training failure")
                        self.faiss_index = faiss.IndexFlatL2(self.embedding_dim)
                    except Exception as e2:
                        logger.error(f"Failed to create fallback flat index: {e2}")
                        return added_count
            
            # Add embeddings to index only if trained
            try:
                if isinstance(self.faiss_index, faiss.IndexIVFFlat) and not self.faiss_index.is_trained:
                    logger.error("Cannot add vectors to untrained index. Check the training step.")
                    return added_count
                    
                self.faiss_index.add(embeddings_np)
                self.paper_ids_in_index.extend(ids_to_add)
                logger.info(f"Added {len(embeddings_to_add)} embeddings to FAISS index. Total size: {self.faiss_index.ntotal}")
            except Exception as e:
