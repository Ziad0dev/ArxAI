...
class KnowledgeManager:
    ...
    def identify_research_frontiers(self):
        ...
        # FALLBACK: If no frontiers identified yet, use most frequent concepts from the most recent papers
        if not frontier_concepts and len(self.kb.papers) > 0:
            logger.info("No frontiers identified through standard methods. Using concept frequency fallback.")
            
            # Count all concepts
            concept_counts = Counter()
            for paper_id, paper in self.kb.papers.items():
                concept_counts.update(paper.get('concepts', []))
                
            # Add top concepts by frequency
            top_concepts = [c for c, _ in concept_counts.most_common(10)]
            frontier_concepts.update(top_concepts)
            
            # Also add any concepts from clusters
            if self.concept_clusters:
                for cluster_key, cluster in self.concept_clusters.items():
                    frontier_concepts.add(cluster_key)  # Add the key concept from each cluster
                    if len(frontier_concepts) >= 10:
                        break
            
            logger.info(f"Added {len(frontier_concepts)} frontiers using frequency fallback")
                
        # EXTREME FALLBACK: If still no frontiers and we have papers, just take the first concept from each paper until we have enough
        if not frontier_concepts and len(self.kb.papers) > 0:
            logger.info("Using extreme fallback to identify frontiers from paper concepts")
            for paper_id, paper in self.kb.papers.items():
                concepts = paper.get('concepts', [])
                if concepts:
                    frontier_concepts.add(concepts[0])  # Take the first concept from each paper
                if len(frontier_concepts) >= 5:
                    break
            
        # Create structured frontier objects
        for concept in frontier_concepts:
            # Compute a basic importance score even when we don't have growth or citation data
            base_importance = 0.5  # Default importance
            papers_with_concept = [pid for pid, p in self.kb.papers.items() if concept in p.get('concepts', [])]
            
            frontier = {
                'concept': concept,
                'growth_rate': concept_growth.get(concept, 0.0),
                'citation_importance': citation_importance.get(concept, 0.0),
                'frequency': len(papers_with_concept),
                'cluster': next((cluster for leader, cluster in self.concept_clusters.items() 
                                 if concept in cluster), []),
                'papers': papers_with_concept[:5]  # Limit to 5 papers
            }
            frontiers.append(frontier)
        
        # Sort by combined importance
        for f in frontiers:
            # Use frequency as a factor when other metrics are missing
            f['importance'] = (
                f['growth_rate'] * 0.5 + 
                f['citation_importance'] * 0.3 + 
                (f['frequency'] / max(1, len(self.kb.papers))) * 0.2
            )
